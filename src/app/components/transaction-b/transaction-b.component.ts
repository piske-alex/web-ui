...
export class TransactionBComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.i18ns.reach_maximun_unfinish_order_merchant = await this.languageService.get('transaction.reach_maximun_unfinish_order_merchant');
  ...
  confirmTransaction() {
    ...
    this.adService.transaction({ adid: this.data.adId, amount: this.receiveAmount , coin_amount: this.payAmount }).then(async (data) => {
      ...
      if (error.error === 'Insufficient balance') {
        ...
        this.dialogService.alert(this.i18ns.not_trans_as_ad_hidden);
      } else if (error.error === 'reach_maximun_unfinish_order_merchant') {
        this.dialogService.alert(this.i18ns.reach_maximun_unfinish_order_merchant);
      } else {
        if (error.error) {
